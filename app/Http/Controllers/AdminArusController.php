...
class AdminArusController extends Controller {
    ...
    public function index(Request $request) {  
		...
		$list = User::where(['type' => 'arus'])->orderBy('id', 'desc')->get();
		$search = '';
		...
		if ($request->input('query')) {
			...
			$list = User::where(['type' => 'arus'])->where('name', 'LIKE', '%'.$sq.'%');
	...
	public function add(Request $request) {
		...
		if ($request->isMethod('post')) {
			...
			return redirect('/admin/arus')->with('success', 'Арендодатель добавлен');
		...
		$return = [
			...
			'page_title' => 'Добавить арендодателя',
			'rec' => (object)$rec,
		...
		return view('arus_form', $return);
		
	}
	
	public function edit($id, Request $request) {
				
		$rec = User::where(['id' => $id, 'type' => 'arus'])->first();	
		if (!$rec) {
			return redirect('/admin/arus')->with('error', 'Арендодатель не найден!');
		}
		...
		if ($request->isMethod('post')) {
			...
			return redirect('/admin/arus')->with('success', 'Арендодатель обновлен');
		...
		$return = [
			...
			'page_title' => 'Редактировать арендодателя',
			'rec' => $rec,
			'id' => $id,
			'regions' => Region::orderBy('name', 'asc')->get(),
			'cities' => City::orderBy('name', 'asc')->get(),
