									...
									@if ($list->count())
										@foreach ($list as $key => $record)
											<tr class="footable-odd">
												...
												<td class="footable-visible">{{ $record->clothes }}</td>
												<td class="footable-visible">
													@if ($record->type == 'like')
														<span class="badge badge-success">Лайк</span>
													@else
														<span class="badge badge-danger">Дизлайк</span>
													@endif
												</td>
												<td class="footable-visible">{{ date('d.m.Y - H:i', strtotime($record->created_at)) }}</td>																								
												<td class="text-right footable-visible footable-last-column">
													<div class="btn-group">
														<a href="/admin/delete_record/likes/{{ $record->id }}" class="btn-white btn btn-xs"><i class="fa fa-close"></i></a>
													</div>
												</td>
											</tr>
										@endforeach
